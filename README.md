...
STEP 1-14: 実践レッスン
  GitHub、CI、E2E、PR、サブエージェント、スキル、MCPまで順番に体験したい人向け
```

最初に、どちらで進めるかをAIエージェントに伝えてください。
迷う場合は、初回は `STEP 1-14: 実践レッスン` の `A: じっくり説明` を推奨します。
取得直後の状態確認だけなら `./tools/lesson status` と `./tools/lesson14 status` の両方を使えますが、学習開始前に必ず学習コースを選びます。

## 学習モードと言語を選ぶ

`STEP 1-7: 基礎レッスン` と `STEP 1-14: 実践レッスン` では、説明量を3段階から選びます。
...
`STEP 1-7: 基礎レッスン` は `./tools/lesson`、`STEP 1-14: 実践レッスン` は `./tools/lesson14` で記録します。
...
```text
このリポジトリの index-14-days.md を読み、STEP 1-14: 実践レッスンを順番どおりに進めてください。
...
入口ファイルは、`STEP 1-7: 基礎レッスン` が `index.md`、`STEP 1-14: 実践レッスン` が `index-14-days.md` です。
...
```text
index.md                         STEP 1-7: 基礎レッスンの入口
index-14-days.md                 STEP 1-14: 実践レッスンの入口
