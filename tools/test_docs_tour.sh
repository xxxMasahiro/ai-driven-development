...
require_output "status product agent distinction" "$status_output" "product-side AGENT.md"
...
require_output "AGENTS distinction" "$all_output" "AGENTS.MD.*AGENT.md|AGENT.md"
...
require_output "dashboard product agent distinction" "$dashboard_output" "product-side AGENT.md"
require_output "dashboard failure memory" "$dashboard_output" "product-side FAILURE_MEMORY.md"
...
require_pattern "prompts/PROMPTS.md" "AGENT.mdは成果物リポジトリ側" "7-day AGENT distinction prompt"
require_pattern "prompts/PROMPTS.md" "成果物リポジトリ側のFAILURE_MEMORY.md" "7-day product failure memory prompt"
require_pattern "prompts/PROMPTS_14_DAYS.md" "文書マップ|DOCUMENT_MAP" "14-day document-map prompt"
require_pattern "prompts/PROMPTS_14_DAYS.md" "AGENT.mdは成果物リポジトリ側" "14-day AGENT distinction prompt"
require_pattern "prompts/PROMPTS_14_DAYS.md" "成果物リポジトリ側のFAILURE_MEMORY.md" "14-day product failure memory prompt"
...
if grep -Eq 'planned and not yet implemented|planning-synchronization stage|not yet expected to exist|not yet part of the runtime|next additive implementation plan|must be completed only after' "$ROOT/docs/as-built/SPECIFICATION.md" "$ROOT/docs/as-built/REQUIREMENTS.md" "$ROOT/docs/as-built/IMPLEMENTATION_PLAN.md" "$ROOT/docs/workflow/TASK_TRACKER.md" "$ROOT/docs/workflow/HANDOFF.md"; then
