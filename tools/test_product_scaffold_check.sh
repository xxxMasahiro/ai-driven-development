...
legacy_only="$TMP_DIR/legacy-only"
make_repo "$legacy_only"
mkdir -p "$legacy_only/ops" "$legacy_only/src" "$legacy_only/tests"
printf '# Agent\n' >"$legacy_only/AGENT.md"
printf '# Product\n' >"$legacy_only/README.md"
printf '# Requirements\n' >"$legacy_only/REQUIREMENTS.md"
printf '# Specification\n' >"$legacy_only/SPECIFICATION.md"
printf '# Implementation Plan\n' >"$legacy_only/IMPLEMENTATION_PLAN.md"
printf '# Task Tracker\n\n## Current Status\nReady.\n\n## Remaining Work\n- Continue.\n\nHANDOFF\n' >"$legacy_only/TASK_TRACKER.md"
printf '# Handoff\n\n## Current State\nReady.\n\n## Next Step\n- Continue.\n\nTASK_TRACKER\n' >"$legacy_only/HANDOFF.md"
printf 'source\n' >"$legacy_only/src/app.js"
printf 'test\n' >"$legacy_only/tests/app.test.js"
printf 'entry\n' >"$legacy_only/index.html"
write_common_manifests "$legacy_only"
cat >"$legacy_only/ops/PRODUCT_MANIFEST.tsv" <<'DOC'
# authority_id	required_mode	contexts	product_types	path	path_role	validation_rule	dashboard_group	description
product.entrypoint	required	all	all	index.html	entrypoint	file_exists	workflow	Browser entrypoint.
product.source	required	all	all	src/app.js	source	file_nonempty	workflow	Application source authority.
product.test	required	all	all	tests/app.test.js	test	file_nonempty	workflow	Test authority.
DOC
"$ROOT/tools/product-scaffold-check" check --repo "$legacy_only" --context free-development \
  >/tmp/product-scaffold-legacy-only.out 2>&1 && exit 1 || true
grep 'canonical path for product_docs.requirements' /tmp/product-scaffold-legacy-only.out >/dev/null
